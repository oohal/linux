...
static struct pci_dev *eeh_rescan_pci_dev(struct pci_bus *bus, int devfn)
{
	 ...
	 * device without the error callbacks into a known-good state. For
	 * bridges they should either be implementing the error callbacks
	 * (i.e pcieport) or they should be restorable by the platform APIs so
	 * EEH doesn't remove them.
	 *
			    ...
			    struct eeh_rmv_data *rmv_data)
{
	...
	// oliver's handy-dandy guide to whatever the fuck is going on in here:
	//
	// step 1. Remove pci_dev's which don't have
	//
	// After eeh_rmv_device() is called for a device:
	//
	// * eeh_dev->flags has EEH_DEV_DISCONNECTED set
	// * eeh_dev->pdev is invalid
	// * eeh_dev is still on the parent PE lists
	//
	...
	//
	// FIXME: er, so we haven't recovered VFs at this point.
