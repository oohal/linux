...
eeh_find_all_pfs() {
	...
	for dev in `ls -1 /sys/bus/pci/devices/` ; do
		if [ ! -e "/sys/bus/pci/devices/$dev/sriov_numvfs" ] ; then
			continue
		fi
		...
		# on PowerNV totalvfs==0 means we can't map the IOV BARs
		if [ "$(cat /sys/bus/pci/devices/$dev/sriov_totalvfs)" == "0" ] ; then
			continue
		fi
